...
// FUNCION PARA EL MÉTODO DE BISECCIÓN //
export function BisectionMethod(
    ...
    const headerRow = ["Iteración", "a", "c", "b", "f(a)", "f(c)", "f(b)", "Ec%"];
    ...
    new Chart(ctx, {
        ...
        data: {
            ...
            datasets: [{
                label: 'Valor de c por iteración',
                data: eachCenter,
        ...
        options: {
            ...
            scales: {
                ...
                y: { 
                    title: { display: true, text: 'Valor de c' },
                    ticks: {
...
function BisectionOperation(
    ...
    const result = BisectionCalculus(
        aInterval,
        bInterval,
        mathFunction,
        epsilon,
    ...
    BisectionLog(
        ...
        result.aEvaluation,
        result.bEvaluation,
        result.centerEvaluation,
        result.aIsLessThanZero,
        result.bIsLessThanZero,
        result.centerIsLessThanZero
    );
    ...
    sheet.addRow([
        ...
        result.aEvaluation,
        result.centerEvaluation,
        result.bEvaluation, { formula: excelFormula }
    ]);
    ...
    if (iteration === 1) {
        ...
        relativeError = math.abs(((lastCenter - result.center) / result.center));
    }
    ...
    const data = [
        ...
        formatNumber(result.center),
        formatNumber(bInterval),
        formatNumber(result.aEvaluation),
        formatNumber(result.centerEvaluation),
...
function BisectionCalculus(
    ...
    if (aEvaluation * bEvaluation > 0) {
        alert("Intervalo inválido: no hay cambio de signo en [a, b]");
        iterate = false;
    }

    let center = (aInterval + bInterval) / 2;

    let { lessThanZero: centerIsLessThanZero, result: centerEvaluation } = evaluateFunction(mathFunction, center);

    if (aEvaluation === 0 || centerEvaluation === 0 || bEvaluation === 0) {
        alert(`Se encontró la raíz exacta en la iteración ${iteration}...`);
        iterate = false;
    }
    else if (Math.abs(centerEvaluation) <= epsilon) {
        alert(`El valor de la raíz "${center}" cumple con la precisión deseada en la iteración ${iteration}...`);
        iterate = false;
    }
    ...
    if (!iterate) {
        return {
            ...
            aIsLessThanZero,
            bIsLessThanZero,
            centerIsLessThanZero,
            iterate
        };
    }

    if (aIsLessThanZero !== centerIsLessThanZero) {
        bInterval = center;
    }
    else {
        aInterval = center;
...
function BisectionLog(
    ...
    console.log("Fórmula: (a -  b) / 2");
    console.log(`Cálculo del punto medio: (${aInterval} - ${bInterval}) / 2`);

    console.log("");
    console.log(`A - evaluación: f(x) = ${mathFunction} // f(x) = ${aEvaluation}`);
    console.log(`B - evaluación: f(x) = ${mathFunction} // f(x) = ${bEvaluation}`);
    console.log(`C - evaluación: f(x) = ${mathFunction} // f(x) = ${centerEvaluation}`);
    
    console.log("");

    console.log(`A - es menor que 0: ${aIsLessThanZero}`);
    console.log(`C - es menor que 0: ${centerIsLessThanZero}`);
